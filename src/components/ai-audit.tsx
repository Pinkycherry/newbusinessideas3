...
export function AiAudit({ slug, locked = false }: { slug: string; locked?: boolean }) {
  ...
  return (
    <section className="glass mt-10 rounded-3xl px-5 py-7 sm:px-8">
      ...
      {audit.isPending && (
        <div className="mt-7 border-t border-border pt-6" aria-live="polite" aria-busy="true">
          <div className="flex flex-wrap items-center gap-4">
            <div className="iv-skeleton h-20 w-20 rounded-2xl" />
            <div className="min-w-[12rem] flex-1 space-y-2">
              <div className="iv-skeleton h-4 w-4/5" />
              <div className="iv-skeleton h-4 w-3/5" />
            </div>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="space-y-2 rounded-2xl border border-border p-4">
                <div className="iv-skeleton h-3 w-24" />
                <div className="iv-skeleton h-3.5 w-full" />
                <div className="iv-skeleton h-3.5 w-11/12" />
                <div className="iv-skeleton h-3.5 w-2/3" />
              </div>
            ))}
          </div>
        </div>
