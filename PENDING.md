...
| 34 | **Inline internal linking — trial live on one idea page only, needs a go/no-go** | Site-wide audit (2026-09-24) confirmed no idea page had inline natural-sentence internal links before this; the only precedent anywhere on the site was 14 blog posts with 1 bare `<a>` each, no styling, no friendly copy. Built: 3 curated `InternalLink` slots (breakdown/playbooks/closing, read from `ideas.internal_link_anchors`, only IDEA-00330 has real data) + a 4th fully-automatic "One more thing, from us to you" section (2 links auto-picked from `related`/`trending`, works on every idea with zero manual data entry). Both are gated to `part-time-podcast-guest-pitch-writing-business` only pending review — see it live at that URL. Saffron/yellow-hover styling; fixed a real bug where `html.light .bbi-instrument a:hover{!important}` was overriding the hover color to white (commit `c5660d6`). | **You** decide: roll the 4th section out to all ideas (no data work needed), and whether to resume writing curated `internal_link_anchors` content for other ideas or drop that approach |
