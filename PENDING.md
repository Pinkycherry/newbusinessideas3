...
| 36 | **Curated internal-link slots (breakdown/playbooks/closing) still only have real data on IDEA-00330** | Approved and shipped 2026-09-24: the "One more thing, from us to you" friend-talk section (2 auto-picked links, no manual data needed) now runs on every idea page — verified live on a random idea, not just the trial one. The 3 OLDER curated `InternalLink` slots reading `ideas.internal_link_anchors` are separate and still only populated for IDEA-00330 (self-gate by data presence elsewhere, render nothing). Saffron/yellow-hover styling; a real bug where `html.light .bbi-instrument a:hover{!important}` overrode the hover color to white was found and fixed live (commit `c5660d6`). | **You** decide: resume writing curated `internal_link_anchors` content for the rest of the library, or drop that approach now that the automatic section covers the same job |
