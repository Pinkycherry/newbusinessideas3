...
| 37 | **Cinema trial on two idea pages: waiting on your review before any rollout** | `solar-fencing-business-for-farms` and `part-time-podcast-guest-pitch-writing-business` render through `src/components/idea-cinema/` (allowlist `CINEMA_TRIAL_SLUGS` in `idea.$slug.tsx`). Round two (2026-09-24) stripped the decorative styling the founder called AI slop, fixed the mobile hero order, unified section navigation and moved page-only styles into `idea-cinema/cinema.css`; fold that file into the trial block in `styles.css` at rollout. Rollback ref: branch `rollback/pre-cinema-trial-2026-09-24` (= `8ae01b3`). Found while profiling, **site-wide, not fixed outside the trial** (the rollout should carry these): the root pointer channel restyles the whole document every pointer frame (`--ptr-*` on `:root`); `usePageScrollProgress` does the same on every scroll frame; every `.bbi-instrument` button runs an infinite `@property` border animation (`bbi-edge-travel`), which repaints on the main thread; the header's "Browse free" `HoverBorderGradient` keeps framer's frame loop running forever; the sticky header never sticks because `body { overflow-x: hidden }` makes body a scroll container (fix: `overflow-x: clip`); the header's wrapper is transparent, so text shows through around the nav card; a mobile-only `!important` rule inflates every unsized `h2`; the nav dropdowns have no Escape handling; `ShareLinks` leaves collapsed actions keyboard-focusable; `trend_score` is shown everywhere with no note that it is an internal editorial score. Each is fixed inside the trial scope. | **You** review both pages; then Claude rolls out |
