...
### 11.1 `seo_title` is mechanically generated and reads as broken — MAJOR, fix ASAP

**Status: open. Highest-priority SEO defect on the site. Deliberately not fixed
before the trial launch; to be fixed as soon as the site is up.**

`seo_title` is populated on all 409 rows and `src/routes/idea.$slug.tsx:112`
renders it as the `<title>` tag ahead of `title`. So this string — not the
idea's display name — is **what Google prints as the blue link in search
results, and what a person reads when deciding whether to click.**

It was generated by title-casing the slug and appending "Business Path", which
breaks on two counts:

| Slug                                     | Current `seo_title`                                    | Defect                                   |
| ---------------------------------------- | ------------------------------------------------------ | ---------------------------------------- |
| `white-label-software-business`          | `White Label Software Business Business Path`          | "Business Business"                      |
| `ai-chrome-extension-business`           | `Ai Chrome Extension Business Business Path`           | "Business Business", and `Ai` not `AI`   |
| `data-enrichment-api-business`           | `Data Enrichment Api Business Business Path`           | "Business Business", and `Api` not `API` |
| `subscription-billing-software-business` | `Subscription Billing Software Business Business Path` | "Business Business", 52 chars            |

**Measured: 188 of 409 rows have a slug ending in `-business`**, so the
duplicated word affects at least that many. Acronym mis-casing (`Ai`, `Api`,
`Saas`, `Seo`, `B2b`, `Ugc`) is a separate count still to be taken.

This is a validated finding with a query behind it, not an estimate.

**The fix, when it runs:** rewrite the one column. No slug changes, no URL
changes, no redirects, no code change, no n8n change. Derive the title from the
slug, strip the duplicated word, fix acronym casing, lower-case joining words
(`for`, `to`, `and`, `with`), and cap the length so Google does not truncate:

```
white-label-software-business   → White Label Software Business Ideas | BBI
ai-chrome-extension-business    → AI Chrome Extension Business Ideas | BBI
software-for-retail-shop-owners → Software for Retail Shop Owners | BBI
```

Check `meta_description` in the same pass — it is also populated on all 409 and
was probably generated by the same rule, so it likely carries the same
duplication. The description is the other half of what a person sees before
they click.

**This is a Supabase write to a table n8n also writes to.** It needs an explicit
go-ahead, and a twenty-row before/after preview should be reviewed first.
